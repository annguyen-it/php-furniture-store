...
class Products
{
	...
	public function addProduct(
		...
		$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_brand`, `product_title`, `product_qty`, `product_price`, `product_desc`, `product_keywords`) VALUES ('$category_id', '$brand_id', '$product_name', '$product_qty', '$product_price', '$product_desc', '$product_keywords')");
		...
		$files_array = $this->reArrayFiles($files);
		$pid = $this->con->insert_id;

		var_dump($pid);

		foreach ($files_array as $file) {
			try {
				$this->uploadImage($pid, $file);
			} catch (Exception $e) {
				var_dump($e);
			}
		}
	...
	private function uploadImage($pid, $file)
	{
		$fileName = $file['name'];
		...
		$q = $this->con->query("INSERT INTO product_image (product_image, product_id) VALUES ('$uniqueImageName', $pid)");
